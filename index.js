...
// Update: Fixed Via Browser/Mobile touch events, Reverted to Flex layout with padding for stability
...
function injectCustomStyles() {
    ...
    const css = `
        ...
        /* Modal Overlay - 回归 Flex 布局，最稳健 */
        #img-router-modal-overlay { 
            position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
            background: rgba(0,0,0,0.6); backdrop-filter: blur(3px); 
            z-index: 2147483646; 
            display: none; 
            /* 关键：Flex 布局 + Padding Top 实现顶部对齐 */
            display: flex;
            align-items: flex-start; 
            justify-content: center;
            overflow-y: auto; /* 允许遮罩层滚动 */
            padding-top: 120px; /* 避开顶部，强制下移 */
        ...
        #img-router-modal { 
            ...
            width: 90%; 
            max-width: 550px; 
            /* 不再使用 margin 定位，完全依赖 overlay 的 padding */
            margin: 0; 
...
// === 核心修复：独立封装 Toggle 函数，不依赖 click 事件 ===
...
function initFabDrag(fabElement) {
    ...
    function handleStart(e) {
        if (e.type === 'touchstart') e.preventDefault(); // 必须阻止默认，否则会触发滚动
    ...
    function handleEnd(e) {
        ...
        // 核心修复：如果是触摸结束且没有移动，直接调用 toggleModal
        if (e.type === 'touchend' && !hasMoved) { 
            e.preventDefault(); 
            toggleModal(); // 直接调用，不模拟 click
        }
            ...
            <div id="img-router-modal">
                ...
                <div id="img-router-modal-content" style="padding: 15px;">
                    <div class="img-router-header">
                        <h3 style="margin:0;">🎨 图像生成器 <span style="font-size:0.6em; opacity:0.7;">v2.2.0</span></h3>
