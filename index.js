...
// Supports text-to-image, image-to-image, history gallery, prompt prefix, and fixed reference mode
...
const defaultSettings = {
    ...
    referenceImages: [],
    generatedHistory: [],
    fixReferenceImages: false // 新增：是否固定参考图
};
...
function loadSettings() {
    ...
    $('#img-router-fix-ref').prop('checked', extension_settings[extensionName].fixReferenceImages); // 加载固定开关状态
...
function clearAllImages() {
    ...
    updateImagePreviews();
    // toastr.info('参考图已清空'); // 自动清空时不弹窗，手动点才弹窗？这里复用逻辑，暂不弹窗以免打扰
}
...
async function generateImage(prompt, referenceImages = null) {
    ...
    try {
        ...
        // === 关键逻辑：生成成功后，根据设置决定是否清空参考图 ===
        if (content && images.length > 0) {
            if (!settings.fixReferenceImages) {
                console.log('[img-router] Auto-clearing reference images (Single mode)');
                clearAllImages();
            } else {
                console.log('[img-router] Keeping reference images (Fixed mode)');
            }
        }
...
function injectCustomStyles() {
    ...
    const css = `
        ...
        .img-router-upload-zone { border: 2px dashed #555; padding: 15px; text-align: center; border-radius: 6px; cursor: pointer; transition: 0.2s; }
            ...
            <div id="img-router-modal">
                ...
                <div id="img-router-modal-content" style="padding: 15px;">
                    <div class="img-router-header" style="border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; padding-bottom: 10px;">
                        <h3 style="margin:0;">🎨 图像生成器 <span style="font-size:0.6em; opacity:0.7;">v1.6.0</span></h3>
                    </div>
