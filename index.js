...
async function generateImage(prompt, referenceImages = null) {
    ...
    try {
        ...
        console.log('[img-router] Raw API Response Content:', content);
        return content;
    } catch (error) {
...
/**
 * 增强版图片提取函数
 * 能够识别 Markdown、纯 URL、JSON 中的 URL
 */
function extractImageFromContent(content) {
    if (!content) return null;

    // 1. 尝试匹配 Markdown 格式 ![alt](url)
    const mdMatch = content.match(/!\[.*?\]\((.*?)\)/);
    if (mdMatch) {
        console.log('[img-router] Extracted via Markdown:', mdMatch[1]);
        return mdMatch[1];
    }

    // 2. 尝试匹配纯 URL (http/https 开头，图片后缀结尾)
    // 宽松模式：只要包含 http 且看起来像图片
    const urlMatch = content.match(/(https?:\/\/[^\s"']+\.(?:png|jpg|jpeg|webp|gif|bmp))/i);
    if (urlMatch) {
        console.log('[img-router] Extracted via URL Regex:', urlMatch[0]);
        return urlMatch[0];
    }

    // 3. 尝试匹配 Base64 (data:image/...)
    const base64Match = content.match(/(data:image\/[^;]+;base64,[^"\s]+)/);
    if (base64Match) {
        console.log('[img-router] Extracted via Base64');
        return base64Match[1];
    }
    
    // 4. 兜底：如果整个内容看起来就是一个 URL (忽略首尾空白)
    const trimmed = content.trim();
    if (trimmed.startsWith('http://') || trimmed.startsWith('https://')) {
        // 简单的过滤，防止包含多余文字
        if (!trimmed.includes(' ') && !trimmed.includes('\n')) {
            console.log('[img-router] Extracted via Raw String');
            return trimmed;
        }
    }

    console.warn('[img-router] Failed to extract image from content:', content);
    return null;
}
        ...
        try {
            ...
            if (content) {
                ...
                if (imageUrl) {
                    ...
                    throw new Error('无法从返回内容中提取图片 URL，请按 F12 查看控制台日志');
                }
            } else {
                ...
                <div id="img-router-modal-content">
                    <div id="img-router-settings" class="img-router-panel" style="padding: 15px;">
                        <div class="img-router-header" style="border-bottom: 1px solid #444; margin-bottom: 15px; padding-bottom: 10px;">
                            <h3 style="margin:0;">🎨 图像生成器 <span style="font-size:0.6em; opacity:0.7;">v1.4.0</span></h3>
                        </div>
